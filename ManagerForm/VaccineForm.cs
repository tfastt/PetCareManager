...
namespace ManageForm
{
    public partial class VaccineForm : Form
    {
        ...
        private VaccineRepository _vaccineRepo = new VaccineRepository();
        VaccinationScheduleRepository _repo = new VaccinationScheduleRepository();
        ...
        {
            var vaccineList = _vaccineRepo.GetAllVaccines();
        ...
        private void LoadVaccineGrid()
        {
            ...
            dgvVaccines.Columns.Add("ScheduleID", "ID");
            ...
            var list = _repo.GetAllSchedules()
                            .Where(x => x.PetID == _petId)
                            .ToList();

            foreach (var v in list)
            {
                var vaccineName = vaccineList
                    .FirstOrDefault(x => x.VaccineID == v.VaccineID)?.VaccineName;

                dgvVaccines.Rows.Add(
                    v.ScheduleID,
                    vaccineName, 
                    v.VaccinationDate.ToString("dd/MM/yyyy"),
                    v.NextVaccinationDate?.ToString("dd/MM/yyyy") ?? "",
                    v.Note
                );
            }
        }
